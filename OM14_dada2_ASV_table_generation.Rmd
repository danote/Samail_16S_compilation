...
subtitle: "Source file: OM14_ASV_table_generation.Rmd"
...
````{r }
fnFs <- sort(list.files(demultiplex_path, pattern="R1_", full.names = TRUE))
fnRs <- sort(list.files(demultiplex_path, pattern="R2_", full.names = TRUE))
...
flags_figaro <- paste("-i", paste0("file:", barcode_path_OM14), "-o", file.path(demultiplex_path, "{name}_R1.fastq.gz"), "-p", file.path(demultiplex_path, "{name}_R2.fastq.gz"), R1_path_OM14, R2_path_OM14)

(cutadapt_messages_1 <- system2(cutadapt_path, args = flags_figaro, stdout = TRUE, stderr = TRUE)) 
```
...
Assign the primers you used to "FWD" and "REV" below. Note primers should be not be reverse complemented ahead of time. Our tutorial data uses 514f and 806br those are the primers below. Change if you sequenced with other primers.
...
FWD <- "GTGYCAGCMGCCGCGGTAA"  ## CHANGE ME # this is 514f
...
| <span> |
| :--- |
| **WARNING:** THESE PARAMETERS ARE NOT OPTIMAL FOR ALL DATASETS. Make sure you determine the trim and filtering parameters for your data. The following settings are generally appropriate for MiSeq runs that are 2x140 bp. See above for more details. |
| <span> |
...
filt_out <- filterAndTrim(fwd=file.path(subF.fp, fastqFs), filt=file.path(filtpathF, fastqFs),
              ...
              truncLen=c(140,140), maxEE=1, truncQ=11, maxN=0, rm.phix=TRUE,
