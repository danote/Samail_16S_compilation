---
title: "Demultipexing and adapter trimming of 16S rRNA gene sequences of DNA from Oman samples collected in 2015"
...
if (!dir.exists(project_path_OM15)) dir.create(project_path_OM15)
...
```{r demux}
# 2015 barcode length is 12 bases. At error rate of 10% (cutadapt default), there is 1 allowed error.
...
flags_cutadapt_1 <- paste("-e", error_rate_cutadapt, "--no-indels", "-m", minimum_length_cutadapt_1, "--overlap", minimum_overlap_cutadapt_1, "-g",  paste0("file:", barcode_path_OM15), "-o", file.path(demultiplex_assigned_path, "{name}_R1.fastq.gz"), "-p", file.path(demultiplex_assigned_path, "{name}_R2.fastq.gz"), "--untrimmed-output", file.path(demultiplex_unassigned_path, "unassigned_R1.fastq.gz"), "--untrimmed-paired-output", file.path(demultiplex_unassigned_path, "unassigned_R2.fastq.gz"), R1_path_OM15, R2_path_OM15)
